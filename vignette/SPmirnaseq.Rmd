...
writeTargetsout(x = WF, file = "default", step = 1, new_col = "FileTrim", new_col_output_index = 1, 
    overwrite = TRUE)
dir_path <- "param/cwl/fastqc/" 
WF <- loadWorkflow(targets = "targets_cutadapt.txt", wf_file = "fastqc.cwl", input_file = "fastqc.yml", dir_path = dir_path)
WF <- renderWF(WF, inputvars = c(FileTrim= "_FASTQ_PATH1_", SampleName = "_SampleName_"))
WF
output(WF)
cmdlist(WF)
...
targets <- "targets_cutadapt.txt"
dir_path <- "param/cwl/bowtie2/bowtie2-se"
bowtie <- loadWorkflow(targets = targets, wf_file = "bowtie2-mapping-se.cwl", 
...
resources <- list(walltime=120, ntasks=1, ncpus=4, memory=10240) 
reg <- clusterRun(bowtie, FUN = runCommandline, more.args = list(args=bowtie, dir = FALSE), 
