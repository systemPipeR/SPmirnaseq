...
targetspath <- "targets_mirnaseq.txt" ##system
...
```{r cutadapt_prep, eval=FALSE}
dir_path <- "param/cwl/cutadapt/" ##system
...
```{r}
dir_path <- "param/cwl/fastqc/" ##system
...
<center><img src="results/fastqReport_after.pdf"></center>
...
targets <- "targets_cutadapt.txt" ##system
dir_path <- "param/cwl/bowtie2/bowtie2-mi" #system
bowtie <- loadWorkflow(targets = targets, wf_file = "bowtie2-mapping-mi.cwl", 
...
```{r read_stats, eval=FALSE}
showDT(read.table("results/alignStats_bowtie.xls", header = TRUE))## system
```
...
targetspath <- "targets_cutadapt.txt" ##system
dir_path <- "param/cwl/workflow-bwa-aln/" ##system
...
showDT(assays(countDFmiR)$counts)
```
...
# DEG analysis with *`DESeq2`* ## REPETIR BOWTIE!!!
...
cmp[[1]]
degseqDF <- run_DESeq2(countDF = assays(countDFmiR)$counts, targets = targets, cmp = cmp[[1]], 
    independent = FALSE)
```
...
```{r}
DEG_list2 <- filterDEGs(degDF = degseqDF, filter = c(Fold = 0.5, FDR = 20))
```
